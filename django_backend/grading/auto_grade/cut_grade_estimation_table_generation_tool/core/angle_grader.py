...
class DiamondAngleGrader:
    ...
    def __init__(self, table_file_path):
        ...
        self.pavilion_angle_ranges = np.array(
            [float("{:.1f}".format(angle)) for angle in np.arange(38.8, 43.0, 0.2)]
        )

        self._get_input_grades()

        self.pavilion_angle_map_dict = {}
        self.crown_angle_map_dict = {}
        self._map_angles_to_integers()

        self.table = []
        self._generate_table()

    def _get_input_grades(self):
        """
    ...
    def _map_angles_to_integers(self):
        """
