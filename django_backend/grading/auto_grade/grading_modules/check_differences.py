...
def check_differences_print_and_store_log(original_csv_path: str, computed_csv_path: str) -> None:
    ...
    for i, (expected_stone, result_stone) in enumerate(zip(expected, result)):
        ...
        for test_field in test_fields:
            if expected_stone[test_field] != result_stone[test_field]:
                logs.append(dict(test_field=test_field,computed=result_stone,hand_grading=expected_stone,line_number=i+2))
    ...
    if logs:
        ...
        for i, log in enumerate(logs, 1):
            ...
            printlog(index=i, internal_id=internal_id, line_number=line_number, test_field=test_field, computed_grading=computed_grading, hand_grading=hand_grading)
            writelog(output_log_file_path=OUTPUT_LOG, index=i, 
                        internal_id=internal_id, line_number=line_number, 
                        test_field=test_field, computed_grading=computed_grading, 
...
def printlog(index, internal_id, line_number, test_field, computed_grading, hand_grading):
    ...
    print('ID:          ',internal_id,'\n'
          ...
          'Computed:    ',computed_grading,'\n'
          'Hand Graded: ',hand_grading,'\n')
